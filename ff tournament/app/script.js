    ...
    if (isAdmin) {
        ...
        if (dangerResetButton) {
            dangerResetButton.addEventListener('click', () => {
                ...
                window.location.href = '../LOGIN SYSTEMS/Login.html';
            });
        }
        ...
        winButtons.forEach((button) => {
            button.addEventListener('click', () => {
                ...
                userToUpdate.coins = (Number(userToUpdate.coins) || 0) + 10;
                userToUpdate.winCoins = (Number(userToUpdate.winCoins) || 0) + 10;
                userToUpdate.totalEarnings = (Number(userToUpdate.totalEarnings) || 0) + 10;
                saveUsers(usersToUpdate);
                logWalletTransaction(targetEmail, 'Admin credited 10 coins to your account', 10);
                renderDashboard();
            });
        });
    ...
    function renderPaymentDetails(offer) {
        ...
        modalBody.innerHTML = `
            <div class="flex flex-col items-center text-center space-y-4">
                <div class="w-56 h-56 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 flex flex-col items-center justify-center text-gray-400 gap-2">
                    <span class="text-4xl">📷</span>
                    <span class="text-xs font-medium px-4 text-center">Payment QR code goes here<br>(replace with your real QR image)</span>
                </div>
